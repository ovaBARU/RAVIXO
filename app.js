...
async function auth(e){
  ...
  if(authMode==='register'&&(!$('[id=displayName]').value.trim()||!$('#username').value.trim()))return $('#authMessage').textContent='Nama dan username wajib diisi.';
  try{
    const body=authMode==='login'?{email,password}:{email,password,display_name:$('#displayName').value.trim(),username:$('#username').value.trim()};
    ...
    status(authMode==='login'?'Berhasil masuk.':'Akun berhasil dibuat.');
    await loadFeed();
...
function setAuthMode(m){
  ...
  $('#displayNameWrap')?.classList.toggle('hidden',m==='login');
  $('#usernameWrap')?.classList.toggle('hidden',m==='login');
  if($('#authSubmit'))$('#authSubmit').textContent=m==='login'?'Masuk':'Daftar';
  if($('#authPassword'))$('#authPassword').autocomplete=m==='login'?'current-password':'new-password';
