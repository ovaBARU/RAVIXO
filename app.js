...
function handleSelectedMedia(files){
  const input=$('#mediaInput');
  const list=[...(files||[])];
  if(!list.length){selectedMedia=[];previewSelectedMedia([]);return false}
  if(list.length>10){selectedMedia=[];previewSelectedMedia([]);if(input)input.value='';status('Maksimal 10 foto/video sekaligus.',true);return false}
  const postKind=input?.dataset.postKind||'image';
  if(postKind==='image'&&list.some(f=>!f.type.startsWith('image/'))){selectedMedia=[];previewSelectedMedia([]);if(input)input.value='';status('Mode foto hanya menerima gambar.',true);return false}
  if(postKind==='video'&&list.some(f=>!f.type.startsWith('video/'))){selectedMedia=[];previewSelectedMedia([]);if(input)input.value='';status('Mode video hanya menerima video.',true);return false}
  if(list.some(f=>f.size>100*1024*1024)){selectedMedia=[];previewSelectedMedia([]);if(input)input.value='';status('Setiap file maksimal 100 MB.',true);return false}
  selectedMedia=list;previewSelectedMedia(list);return true;
}
function previewSelectedMedia(files){
  const box=$('#mediaPreview');if(!box)return;
  if(!files||!files.length){box.innerHTML='';updatePreviewArrows();return}
  box.innerHTML=files.map((file,i)=>{
    const url=URL.createObjectURL(file);
    return file.type.startsWith('video/')
      ? `<div class="media-preview-item" data-media-index="${i}"><video src="${esc(url)}" controls playsinline></video><small>${i+1}. ${esc(file.name)}</small></div>`
      : `<div class="media-preview-item" data-media-index="${i}"><img src="${esc(url)}" alt="Pratinjau media ${i+1}"><small>${i+1}. ${esc(file.name)}</small></div>`;
  }).join('');
  ensurePreviewArrows();
  updatePreviewArrows();
}
function ensurePreviewArrows(){
  const box=$('#mediaPreview'); if(!box)return;
  let wrap=box.parentElement;
  if(!wrap||!wrap.classList.contains('media-preview-wrap')){
    wrap=document.createElement('div'); wrap.className='media-preview-wrap';
    box.parentNode.insertBefore(wrap,box); wrap.appendChild(box);
  }
  if(!wrap.querySelector('.preview-nav.prev')){
    const prev=document.createElement('button'); prev.type='button'; prev.className='preview-nav prev'; prev.setAttribute('aria-label','Foto atau video sebelumnya'); prev.innerHTML='‹';
    const next=document.createElement('button'); next.type='button'; next.className='preview-nav next'; next.setAttribute('aria-label','Foto atau video berikutnya'); next.innerHTML='›';
    wrap.append(prev,next);
    prev.addEventListener('click',()=>scrollPreview(-1)); next.addEventListener('click',()=>scrollPreview(1));
  }
  if(!box.dataset.navBound){
    box.dataset.navBound='1';
    let sx=0, sy=0, st=0;
    box.addEventListener('touchstart',e=>{const t=e.changedTouches[0];sx=t.clientX;sy=t.clientY;st=Date.now()},{passive:true});
    box.addEventListener('touchend',e=>{const t=e.changedTouches[0];const dx=t.clientX-sx,dy=t.clientY-sy;if(Math.abs(dx)>45&&Math.abs(dx)>Math.abs(dy)){e.preventDefault();scrollPreview(dx<0?1:-1)}},{passive:false});
    box.addEventListener('wheel',e=>{if(Math.abs(e.deltaY)>Math.abs(e.deltaX)){e.preventDefault();box.scrollLeft+=e.deltaY}}, {passive:false});
    box.addEventListener('scroll',updatePreviewArrows,{passive:true});
  }
}
function scrollPreview(direction){
  const box=$('#mediaPreview');if(!box)return;
  const amount=Math.max(180,Math.floor(box.clientWidth*0.82));
  box.scrollBy({left:direction*amount,behavior:'smooth'});
}
function updatePreviewArrows(){
  const box=$('#mediaPreview'); if(!box)return;
  const wrap=box.parentElement; if(!wrap?.classList.contains('media-preview-wrap'))return;
  const prev=wrap.querySelector('.preview-nav.prev'),next=wrap.querySelector('.preview-nav.next'); if(!prev||!next)return;
  const max=Math.max(0,box.scrollWidth-box.clientWidth);
  prev.disabled=box.scrollLeft<=4; next.disabled=box.scrollLeft>=max-4;
...
(async()=>{updateUserUI();await loadMe();await loadFeed();await loadRightChat();if(!currentUser)enforceAuthGate()})();




/* RAVIXO v1.1.7 - fullscreen gallery swipe for posted media */
(function(){
  'use strict';
  const viewer=document.querySelector('#mediaViewer');
  if(!viewer || viewer.dataset.gallerySwipeBound==='1') return;
  viewer.dataset.gallerySwipeBound='1';
  ...
  function end(x,y){
    ...
    sx=sy=0;
    if(Math.abs(dx)>=threshold && Math.abs(dx)>Math.abs(dy) && viewer._galleryItems?.length>1){
  ...
  viewer.addEventListener('pointercancel',()=>{active=false;sx=sy=0},{passive:true});
})();

/* RAVIXO v1.1.5 - robust media preview navigation */
(function(){
  'use strict';
  function setupMediaNavigator(){
    const candidates=[
      document.querySelector('#mediaPreview'),
      document.querySelector('.media-preview'),
      document.querySelector('.selected-media'),
      document.querySelector('.upload-preview'),
      document.querySelector('.preview-list')
    ].filter(Boolean);
    if(!candidates.length) return;
    const box=candidates[0];
    if(box.dataset.ravixoNav==='1') return;
    box.dataset.ravixoNav='1';
    ...
    const items=()=>Array.from(box.children).filter(el=>el.nodeType===1);
    function update(){
      const list=items();
      if(!list.length) return;
      const max=Math.max(0, box.scrollWidth-box.clientWidth);
      const step=Math.max(box.clientWidth*.8, 180);
      let old=box.querySelectorAll('.ravixo-nav');
      old.forEach(x=>x.remove());
      if(max<=4) return;
      const mk=(dir)=>{
        const b=document.createElement('button');
        b.type='button';
        b.className='ravixo-nav ravixo-nav-'+dir;
        b.setAttribute('aria-label',dir==='left'?'Media sebelumnya':'Media berikutnya');
      ...
      box.appendChild(mk('left')); box.appendChild(mk('right'));
    }
    ...
    box.addEventListener('touchend',e=>{
      ...
      if(Math.abs(dx)>45 && Math.abs(dx)>Math.abs(dy)){
        box.scrollBy({left:dx<0?Math.max(160,box.clientWidth*.7):-Math.max(160,box.clientWidth*.7),behavior:'smooth'});
      }
      sx=sy=0;
    },{passive:true});
    const keyHandler=e=>{
      ...
      if(e.key==='ArrowRight'){e.preventDefault();box.scrollBy({left:Math.max(180,box.clientWidth*.8),behavior:'smooth'});}
      if(e.key==='ArrowLeft'){e.preventDefault();box.scrollBy({left:-Math.max(180,box.clientWidth*.8),behavior:'smooth'});}
    };
    document.addEventListener('keydown',keyHandler);
    new MutationObserver(()=>setTimeout(update,30)).observe(box,{childList:true});
    addEventListener('resize',update);
    update();
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',setupMediaNavigator);
  else setupMediaNavigator();
  setInterval(setupMediaNavigator,1200);
})();
